	...
	return (
		<PopoverTrigger asChild>
			<Button
				...
				className={cn(
					"hover:bg-transparent flex h-auto min-h-9 w-fit items-center justify-between gap-2 overflow-hidden rounded-md border border-input bg-transparent px-3 py-1.5 text-sm whitespace-nowrap shadow-xs transition-[color,box-shadow] outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 data-[placeholder]:text-muted-foreground dark:bg-input/30 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-muted-foreground",
					className
				)}
			>
				{children}
...
export function MultiSelectValue({
	...
	return (
		<div
			...
			ref={handleResize}
			className={cn(
				"flex w-full gap-1.5 overflow-hidden items-center",
		...
		>
			{[...selectedValues]
				...
				.map(value => (
					<div
						data-selected-item
						className="px-2 py-1.5 flex items-center gap-1 rounded-md bg-secondary/50 text-sm font-medium hover:bg-secondary/70 cursor-default select-none group relative"
						key={value}
						onClick={
							clickToRemove
								? e => {
